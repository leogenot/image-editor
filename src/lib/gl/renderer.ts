...
export class Renderer {
  ...
  private _pchipTangent(p0: CurvePoint, p1: CurvePoint, p2: CurvePoint): number {
    const h0 = p1[0] - p0[0]
    const h1 = p2[0] - p1[0]
  ...
  private _evalCurve(points: CurvePoint[], t: number): number {
    ...
    const m0 = seg === 0
      ? dk
      : this._pchipTangent(points[seg - 1], points[seg], points[seg + 1])
    const m1 = seg === n - 2
      ? dk
      : this._pchipTangent(points[seg], points[seg + 1], points[seg + 2])
